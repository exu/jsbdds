...
describe('Zonk', function () {
    ...
    it('it puts car and goates into doors', function () {
        zonk.doors.should.have.length(3);
    });

    it('it takes user first choice', function () {
        zonk.answer(1).should.be.true;
    });

    it('remember user second choice', function () {
        zonk.answer(1).should.be.true;
        zonk.choices.should.eql([1]);
        zonk.choice.should.equal(1);
        zonk.answer(0).should.be.true;
        zonk.choices.should.eql([1,0]);
        zonk.choice.should.equal(0);
    });
